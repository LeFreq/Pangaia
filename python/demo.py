...
#pangaia.sf.net  (for Phil and Lucy)
...
import visual         #this pulls in the visual python library, VPython (v5.7)  (http://vpython.org)
import random         #from the Python standard library
from network import * #pangaia module, not really utilized yet
...
p_nodes_lit = 0.07                #light x% of the nodes, chosen randomly.  Try 0%, and then click a dark spot to add a light source.
...
class pangaia(visual):  #has-a Network
    """The Graphical Internet."""

    rate(100)  #sets an upper-bound on how often (per second) the loop runs.

    def __init__(self, title, ):
        self.scene.title = title #UserNodeName or raw_input("Name:") #prompt for user's name if not registered, "think for the future: what name do you want to be addressed as in eternity?"
        ...
        while True:
            ...
            if scene.mouse.events:
                ...
                if mm.pick==None: #create new sphere XXX just testing
                    ...
                    print d[id(mm.pick)], mm.pick.radius
...
class GNode(Node):
    """Graphical Node object in the pangaia Network."""
    ...
    base_color = Pangaia.color.green           #nodes start in the middle of the rainbow
    base_opacity = 0.97                #A function of how much trust there is in the network.  Lower values ==> greater trust

    def __init__(self, network, name, pos, mass, noosphere=0, valence=base_color, lit=False):
        super().__init__()
        network.add(name)
        ...
        sphere(pos=pos, color=valence, opacity=base_opacity, radius=mass, material=materials.emissive if lit else scene.material) #inner sphere  (Note: try material=chrome)
        return sphere(pos=pos, color=atmos(valence), opacity=0.2*base_opacity, radius=mass+noosphere, material=materials.emissive) #outer sphere, color should be converted to hsv and value amplified assuming ambient color is shade of pure white
...
for i in range(num_nodes):
    ...
    obj = node(name, rand3tuple(), random.randint(1,mean_node_radius), base_color, light, random.randint(int(mean_node_radius/2), mean_node_radius*2))    
    d[id(obj)]=name
